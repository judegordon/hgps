...
here; none of it is new evidence.
...
fraction, six intervention types — reading your configurations through a converter
([ADR 0010](decisions/0010-config-v2-and-a-converter.md)) and your data packs unchanged, and writing
the same result CSV. It is a library, a command line and a local web server over one binary
([docs/api.md](api.md), [docs/server-api.md](server-api.md)), on Linux and macOS.
...
twenty seeds, none out of tolerance** ([docs/equivalence.md](equivalence.md)). A comparison is a
hypothesis test per (scenario, year, sex, variable) rather than a tolerance on a number, because two
Monte Carlo runs with different random streams cannot agree exactly; the rules are in
[docs/equivalence-method.md](equivalence-method.md), and the harness is tested both that it passes
when it should and that it fails when it should.
...
example using it cannot be run. And the floor is your CSV's six significant digits.
...
| **B-22** | `std_income` is emitted and never filled — two loops each skip `income` because the other handles it | 0 in all 4,884 rows of a FINCH run |
| **B-26** | a population impact fraction the store cannot supply is a silent no-op; a gap inside a table reads as zero | `KevinHall_PIF`: four of fifteen diseases have no `Smoking` table |
| **B-27** | the published PIF schema has the sex column backwards from the loader and the data | `cervicalcancer` is non-zero only at `Gender=1`, 600 cells of 3,330 |
| **B-21** | immigration into an empty age-sex band abandons that band's target silently | 197 of the baseline's band counts short on `HLM_France`, all of them empty bands |
| **B-05, B-06** | a seed can give a different answer on a different standard library (an income CDF and a linear model over `unordered_map`); an absent seed runs from `std::random_device` and is recorded as `0` | `static_linear_model.cpp:1952`, `mtrandom.cpp:8` |

Eleven more of the same kind are in that table, from `<cctype>` on signed chars to an order-unstable
parallel reduction. This build reproduces none of them, which is why a comparison needs a way to put
them back: each deviation that changes a number has a compatibility flag, and the harness compares
with the flags **on**, then re-runs with them off to report what each is worth
...
series differ and 12,532 agree to the printed precision, including years of life lost,
disability-adjusted life years, head counts, and the prevalence and incidence of eleven diseases.
...
**3.319358 kg**, and the quantile curve gives newborns below it — 3.159 kg for the first person of
one cohort, 2.544 kg for the fifth of another. The baseline dies in the same place on an uncaught
exception, and no seed, cohort size or horizon avoids it ([docs/examples.md](examples.md)).
...
how many impacts it declares and the dynamic model file — right if the alternative is doing nothing
silently, wrong if a policy is *meant* to apply there.
...
`--verbose` prints the largest differences that *passed*, which is how a shift sitting inside an

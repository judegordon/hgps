...
([ADR 0010](decisions/0010-config-v2-and-a-converter.md)) and your packs unchanged.
...
rules, and the harness is tested that it passes when it should and fails when it should.
...
example using it cannot run. And the floor is your CSV's six significant digits.
...
Eleven more of the same kind are in that table. This build reproduces none of them, which is why a
comparison needs a way to put them back: each deviation that changes a number has a compatibility
flag, and the harness compares with the flags **on**, then re-runs with them off to report what each
is worth ([ADR 0041](decisions/0041-deliberate-deviations-are-switchable.md)).
...
series differ and 12,532 agree to the printed precision, including years of life lost, DALYs, head
...
one cohort, 2.544 kg for the fifth of another. No seed, cohort size or horizon avoids it
...
its impacts and the dynamic model file — right if the alternative is doing nothing silently, wrong
if a policy is *meant* to apply there.

...
3. **Nothing is vendored.** The CC BY-NC-ND disease data is fetched on demand. For offline runs and
   ...
   `tools/gen-fixtures`, in the same layout, carrying a `SYNTHETIC.md` that says so. The generator
   writes **two** configurations over that one data store, differing in file layout, output folder
   and name, scenario set, disease set, seed, horizon and age range, and every test that runs a
   configuration runs against both

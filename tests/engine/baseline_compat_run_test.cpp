...
/// @brief A pack's config with `food_labelling` active — the one policy B-24 lives in — and
///        whatever compatibility flags are asked for.
nlohmann::json food_labelling_document(const hgps::test::FixturePack &pack,
                                       const nlohmann::json &compat_flags) {
    ...
    // and one to be wrongly offered it again in. A pack's risk factors are updated in every year
    // but the first, so the policy starts in the run's second year rather than in whichever year
    // the pack itself chose — the first pack's 2012 would leave it only two.
    definition["active_period"]["start_time"] = document["running"]["start_time"].get<int>() + 1;
...
/// Both synthetic packs, because the flag has to reach the engine from whatever configuration a
/// caller has (tests/support/fixture_packs.h).
class BaselineCompatEndToEnd : public hgps::test::FixturePackTest {};

HGPS_TEST_EVERY_FIXTURE_PACK(BaselineCompatEndToEnd);

} // namespace

TEST_P(BaselineCompatEndToEnd, AnOrdinaryRunRecordsNoFlags) {
    const auto outcome = hgps::test::run_simulation(pack().config(), pack_scratch("compat_none"));
...
TEST_P(BaselineCompatEndToEnd, TheManifestNamesTheFlagsTheRunUsed) {
    const auto config =
        hgps::test::write_config_variant(pack(), "compat_manifest_config",
                                         food_labelling_document(
                                             pack(), nlohmann::json::array({"B-24"})));

    const auto outcome = hgps::test::run_simulation(config, pack_scratch("compat_manifest_out"));
    ASSERT_TRUE(outcome.succeeded) << outcome.report.to_string();
...
TEST_P(BaselineCompatEndToEnd, TheFlagChangesTheResultsAndNothingElseDoes) {
    ...
    const auto fixed =
        hgps::test::write_config_variant(pack(), "compat_fixed_config",
                                         food_labelling_document(pack(), nlohmann::json::array()));
    const auto restored =
        hgps::test::write_config_variant(pack(), "compat_restored_config",
                                         food_labelling_document(
                                             pack(), nlohmann::json::array({"B-24"})));

    const auto fixed_run = hgps::test::run_simulation(fixed, pack_scratch("compat_fixed_out"));
    ASSERT_TRUE(fixed_run.succeeded) << fixed_run.report.to_string();
    const auto restored_run =
        hgps::test::run_simulation(restored, pack_scratch("compat_restored_out"));
...
TEST_P(BaselineCompatEndToEnd, TheFlagIsStillDeterministic) {
    ...
    const auto config =
        hgps::test::write_config_variant(pack(), "compat_determinism_config",
                                         food_labelling_document(
                                             pack(), nlohmann::json::array({"all"})));

    const auto first = hgps::test::run_simulation(config, pack_scratch("compat_det_a"), 1);
    const auto second = hgps::test::run_simulation(config, pack_scratch("compat_det_b"), 4);
...
TEST_P(BaselineCompatEndToEnd, TheFlagIsPartOfWhatALoadedConfigurationReports) {
    const auto config =
        hgps::test::write_config_variant(pack(), "compat_reported_config",
                                         food_labelling_document(
                                             pack(), nlohmann::json::array({"B-24"})));

...
void Identifier::validate_identifier() const {
    if (std::isdigit(static_cast<unsigned char>(value_.at(0)))) {
        throw std::invalid_argument("Identifier must not start with a numeric value");
    }

    if (!std::all_of(value_.begin(), value_.end(), [](unsigned char c) {
...
void from_json(const nlohmann::json& j, Identifier& id) {
    id = Identifier{j.get<std::string>()};
}

Identifier operator""_id(const char* id, std::size_t /*unused*/) {
    return {id};
}

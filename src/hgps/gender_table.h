...
template <core::Numerical ROW, core::Numerical TYPE> class GenderTable {
  public:
    ...
    GenderTable(const MonotonicVector<ROW> &rows, const std::vector<core::Gender> &cols,
    ...
    bool empty() const noexcept { return rows_index_.empty() || cols_index_.empty(); }
...
template <core::Numerical TYPE> class AgeGenderTable : public GenderTable<int, TYPE> {
  public:
    ...
    AgeGenderTable(const MonotonicVector<int> &rows, const std::vector<core::Gender> &cols,
                   core::Array2D<TYPE> &&values)

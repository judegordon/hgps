...
class StaticLinearModel final : public RiskFactorAdjustableModel {
  ...
  private:
    ...
    bool has_logistic_model(size_t risk_factor_index) const noexcept;
    ...
    std::vector<double> calculate_income_thresholds(const Population &population) const;
    ...
    void clamp_continuous_income_to_expected_range(RuntimeContext &context) const;
    void assign_continuous_income_categories(RuntimeContext &context) const;
    void process_continuous_income_after_adjustment(RuntimeContext &context) const;

...
const std::string &UnivariateSummary::name() const noexcept { return name_; }
...
double UnivariateSummary::kurtosis() const noexcept {
    ...
    const double k_fact = (moments_[0] - 2.0) * (moments_[0] - 3.0);
    const double n1 = moments_[0] - 1.0;
...
void UnivariateSummary::append(double value) noexcept {
    ...
    const double n = moments_[0];
    const double n1 = n + 1.0;
    const double n2 = n * n;
    const double delta = (moments_[1] - value) / n1;
    const double d2 = delta * delta;
    const double d3 = delta * d2;
    const double r1 = n / n1;

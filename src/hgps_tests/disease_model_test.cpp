...
#include "hgps/default_disease_model.h"
#include "hgps/disease.h"
#include "hgps/disease_table.h"
#include "hgps/relative_risk.h"
...
TEST(TestHealthGPS_Disease, CreateDiseaseTable) {
    ...
    auto data = std::map<int, std::map<core::Gender, DiseaseMeasure>>{
        {1,
         {{core::Gender::male, DiseaseMeasure(std::map<int, double>{{1, 3.14}, {2, 2.13}})},
          {core::Gender::female, DiseaseMeasure(std::map<int, double>{{1, 5.13}, {2, 3.13}})}}},

        {2,
         {{core::Gender::male, DiseaseMeasure(std::map<int, double>{{1, 3.33}, {2, 3.13}})},
          {core::Gender::female, DiseaseMeasure(std::map<int, double>{{1, 3.77}, {2, 7.13}})}}},

        {3,
         {{core::Gender::male, DiseaseMeasure(std::map<int, double>{{1, 3.22}, {2, 2.13}})},
